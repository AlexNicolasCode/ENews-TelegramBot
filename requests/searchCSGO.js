...
request("https://news.google.com/search?q=CSGO&hl=pt-BR&gl=BR&ceid=BR%3Apt-419", function(err, res, body) {
  ...
  function finalLinks(item) {
    ...
    request.get(item, function (err, res, body) {
      ...
      loadLinks.forEach(function(item){
          ...
          linkUrl.push(url + "\n");
          fs.writeFileSync("./requests/links/linksCSGO.txt", linkUrl, (err) => {
              if (err)
                console.log(err);
          });
        });
